...
# Resolve model paths relative to this file so it works on any machine
BASE_DIR = Path(__file__).resolve().parent
skill_extractor_model_path = BASE_DIR / "skill_extractor_model"
tokenizer_path = BASE_DIR / "qg_model_tokenizer"
qg_model_path = BASE_DIR / "qg_model_ml"
...
# Load model - handle early_stopping config issue
try:
    model = T5ForConditionalGeneration.from_pretrained(qg_model_path)
except ValueError as e:
    if "early_stopping" in str(e):
        # Fix generation_config.json before loading
        import json
        gen_config_path = qg_model_path / "generation_config.json"
        ...
        raise
# Ensure generation_config has early_stopping set
...
def generate_interview_questions_api():
    ...
    try:
        ...
        express_url = 'http://localhost:5000/api/questions/store'
        payload = {
